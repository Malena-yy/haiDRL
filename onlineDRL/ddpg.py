...
parser.add_argument('--algo_name', default="ddpg", type=str)
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        action = torch.tanh(self.linear3(x)) * self.action_scale + self.action_bias
        return action
...
class DDPG:
    def __init__(self, args, state_dim=None, action_space=None, writer=None):
        ...
        self.target_actor_net = Actor(state_dim, action_space, args.hidden_dim)
    ...
    def update(self):
        ...
        with torch.no_grad():
            next_state_action = self.target_actor_net(next_state[index])
            next_target_q = self.target_critic_net(next_state[index], next_state_action)
            next_q_value = reward[index] + (1 - terminal[index]) * self.gamma * next_target_q
        q = self.critic_net(state[index], action[index])
        ...
        pi = self.actor_net(state[index])
        q_pi = self.critic_net(state[index], pi)
        ...
        for target_param, source_param in zip(self.target_actor_net.parameters(), self.actor_net.parameters()):
            target_param.data.copy_(
                target_param.data * (1.0 - self.replace_tau) + source_param.data * self.replace_tau)
...
def train_gym(args):
    ...
    if args.use_tensorboard:
        ...
        tf_writer = None
    # 创建智能体
    model = DDPG(args, state_dim, action_space, writer=tf_writer)

    episode = 0
    train_count = 0
    explore_var = 2.  # control exploration
    plt_ep = []
    plt_step = []
    plt_ep_reward = []
    while episode < args.max_episode:
        ...
        while True:
            ...
            action = np.clip(np.random.normal(action, explore_var),  action_space.low,
                          action_space.high)
            ...
            if len(model.buffer) >= 2000:
                ...
                train_count += 1
                explore_var = max([explore_var * .9999, 0.001])
            state = next_state
            # 累加奖励
            ep_reward += reward
            if terminated or truncated:
...
def eval_gym(args):
    ...
    print("run_dir:{}".format(save_path))
    # 创建智能体
    model = DDPG(args, state_dim, action_space)
